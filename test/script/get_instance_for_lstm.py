...
def save_feature(label_info, out_dir):
    ...
    fid_test = open(out_dir + '/test.txt', 'w')
    for res in label_info['results']:
        basename = os.path.basename(res['url']).split('.')[0]
        mode = res['mode']
        print(basename, res['num_proposals'], mode)
        if mode == 'test':
            feat_dir = dataset + '/feature_test'
            fid = fid_test
        else:
            feat_dir = dataset + '/feature_train'
        ...
        # max_len_audio = len(image_feature)
...
if __name__ == "__main__":
    dataset = "/workspace/liukaiyuan/PaddleVideo_Pingpong/aisudio/work/PaddleVideo/applications/FootballAction/datasets/EuroCup2016"
    feat_dir = dataset + '/feature_train'
    prop_file = dataset + '/feature_bmn/prop_all.json'
    out_dir = dataset + '/input_for_lstm'
    label_files = {
        'train': 'label_cls8_train.json',
        'validation': 'label_cls8_val.json',
        'test': 'label_cls8_test.json'
    }
